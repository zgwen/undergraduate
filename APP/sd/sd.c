...
uint8_t SD_WriteCmd(uint8_t cmd, uint32_t dat, uint8_t crc)
{	 
	uint8_t r1 = 0;
	uint16_t i = 0;
	...
	SPI2_WriteReadData(crc);  //这个地方不要添加&0x01否则USB拔掉后下载上电测试就是检测不到SD卡
...
int8_t SD_ReadData(uint8_t *buf)
{
	uint16_t i;
...
int8_t SD_WriteData(uint8_t *buf, uint8_t cmd)
{
	uint16_t i;
...
void SD_pros()
{
	...
	GUI_Show12Char(10,270,"如果没有安装SD卡，请按复位返回主菜单",YELLOW,BLACK);
	...
	GUI_Show12Char(0, 84, "SD card OK                  ", RED, BLACK);
    ...
    if(SD_TYPE == 0x06)
	{
		GUI_Show12Char(0, 145, "SDV2HC OK!", RED, BLACK);
	}
	else if(SD_TYPE == 0x04)
	{
		GUI_Show12Char(0, 145, "SDV2 OK!", RED, BLACK);
	}
	else if(SD_TYPE == 0x02)
	{
		GUI_Show12Char(0, 145, "SDV1 OK!", RED, BLACK);
	}
	else if(SD_TYPE == 0x01)
	{
		GUI_Show12Char(0, 145, "MMC OK!", RED, BLACK);
	}
